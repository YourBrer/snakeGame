...
const canvas: HTMLCanvasElement = document.createElement("canvas"); // Создаем канвас
const ctx: CanvasRenderingContext2D = canvas.getContext("2d")!; // Получаем контекст канваса

canvas.width = root.offsetWidth;
canvas.height = root.offsetHeight;
root.appendChild(canvas);

let prey: Prey = getNewPrey(); // Объект с данными жертвы
let preyColor = getRandomColor(); // Цвет жертвы
let snakeBody: Coords[] = [{ x: 0, y: 0 }]; // Тело змеи
let direction: 'left' | 'right' | 'up' | 'down' = 'right'; // Направление движения
let gameState: 'stop' | 'start' | 'pause' = 'stop'; // Состояние игры - запущена, на паузе, прекращена
let intervalId: number; // id интервала для управления циклом игры
...
function getNewPreyCoords(): [number, number] {
...
function gameLoop() {
  ...
  snakeBody.forEach((snakeBodySegment, ind) => {
    ...
    switch (direction) {
      case 'right':
        if (snakeBodySegment.x > Math.round(canvas.width / SNAKE_CELL_SIZE) * SNAKE_CELL_SIZE) snakeBodySegment.x = 0;
        break;
      case 'up':
        if (snakeBodySegment.y < 0) snakeBodySegment.y = Math.round(canvas.height / SNAKE_CELL_SIZE) * SNAKE_CELL_SIZE;
        break
      case 'left':
        if (snakeBodySegment.x < 0) snakeBodySegment.x = Math.round(canvas.width / SNAKE_CELL_SIZE) * SNAKE_CELL_SIZE;
        break
      case 'down':
        if (snakeBodySegment.y > Math.round(canvas.height / SNAKE_CELL_SIZE) * SNAKE_CELL_SIZE) snakeBodySegment.y = 0;
        break
    }
...
document.addEventListener('keydown', ev => {
  ev.stopPropagation();
  if (ev.code === 'ArrowRight' && direction !== 'left') direction = 'right';
  if (ev.code === 'ArrowUp' && direction !== 'down') direction = 'up';
  if (ev.code === 'ArrowLeft' && direction !== 'right') direction = 'left';
  if (ev.code === 'ArrowDown' && direction !== 'up') direction = 'down';
});
...
speedSelector.onchange = ({ target }: any) => {
